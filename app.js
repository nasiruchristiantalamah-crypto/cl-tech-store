...
// ── EMAILJS CONFIG ─────────────────────────────────────────────
const EMAILJS = {
  serviceId:  "service_b0jwurb",
  ownerTpl:   "template_1c1o0da",  // notification to store owner
  publicKey:  "zfC_TtrWGjxopSVHt",
};

// ── SEND EMAIL TO OWNER (order notification) ──────────────────
async function sendOwnerEmail(order) {
  const itemsList = order.items.map(i =>
    i.name + ' x' + i.qty + ' = GHS ' + fmt(i.price * i.qty)
  ).join('\n');
  ...
  try {
    const result = await emailjs.send(
      ...
      EMAILJS.ownerTpl,
      {
        ...
        to_name:          'Chris & Louisa Tech Store',
        from_name:        'Chris & Louisa Tech Store Website',
        ...
        delivery_type:    order.delivery === 'delivery' ? 'Home Delivery' : 'Store Pickup',
        order_items:      itemsList,
        subtotal:         'GHS ' + fmt(order.subtotal),
        delivery_fee:     'GHS ' + fmt(order.deliveryFee),
        total:            'GHS ' + fmt(order.total),
        notes:            order.notes || 'None',
        message:          'New order received from ' + order.name + '. Total: GHS ' + fmt(order.total),
      }
    );
    console.log('Owner email sent:', result.status);
  } catch(e) {
    console.error('Owner email error:', e.text || e);
  }
}
  ...
  try {
    const result = await emailjs.send(
      ...
      EMAILJS.ownerTpl,
      {
        ...
        to_name:          order.name,
        from_name:        'Chris & Louisa Tech Store',
        ...
        total:            'GHS ' + fmt(order.total),
        notes:            order.notes || 'None',
        message:          'Thank you for your order! We will confirm within 30 minutes.',
      }
    );
    console.log('Customer email sent:', result.status);
  } catch(e) {
    console.error('Customer email error:', e.text || e);
  }
}
...
async function loadProducts() {
  // ── Show products INSTANTLY from cache or demo ────────────────
  const CACHE_KEY = 'clts_products_cache';
  const CACHE_TIME = 'clts_products_time';
  const CACHE_TTL  = 5 * 60 * 1000; // 5 minutes

  // Load from local cache first — instant display
  try {
    const cached = localStorage.getItem(CACHE_KEY);
    const cachedTime = parseInt(localStorage.getItem(CACHE_TIME) || '0');
    if (cached && (Date.now() - cachedTime < CACHE_TTL)) {
      products = JSON.parse(cached);
      renderProducts(); updateStats();
      // Still refresh from Firebase in background silently
      if (CONFIGURED) refreshFromFirebase(true);
      return;
    }
  } catch(e) {}

  // No cache — show demo products immediately while Firebase loads
  if (!CONFIGURED) {
    products = DEMO.map((p,i) => ({id:'d'+i,...p,ts:Date.now()-i*1000}));
    renderProducts(); updateStats(); return;
  }

  // Show demo products right away so page isn't blank
  products = DEMO.map((p,i) => ({id:'d'+i,...p,ts:Date.now()-i*1000}));
  renderProducts(); updateStats();

  // Then load real products from Firebase in background
  refreshFromFirebase(false);
}

async function refreshFromFirebase(silent) {
  try {
    const snap = await db.collection('products').orderBy('ts','desc').get();
    if (snap.empty) {
      // First time — seed demo products
      const seeded = [];
      for (const p of DEMO) {
        const ref = await db.collection('products').add({
          ...p, ts: firebase.firestore.FieldValue.serverTimestamp()
        });
        seeded.unshift({id:ref.id,...p,ts:Date.now()});
      }
      products = seeded;
    } else {
    ...
    try {
      localStorage.setItem('clts_products_cache', JSON.stringify(products));
      localStorage.setItem('clts_products_time', Date.now().toString());
    } catch(e) {}
    renderProducts(); updateStats();
    if (!silent) renderAdminList();
  } catch(e) {
    console.error('Firebase load error:', e);
    // Keep showing whatever products are already displayed
  }
}
...
async function placeOrder() {
  ...
  sendWA(order);               // open WhatsApp to owner
  sendCustomerReceipt(order);  // open WhatsApp to customer

  // Send automatic emails (no button press needed)
  sendOwnerEmail(order);       // instant email to store owner
  sendCustomerEmail(order);    // instant receipt email to customer
  ...
  document.getElementById('checkoutBody').innerHTML = `
    <div class="success-wrap">
      ...
      <p>Your order has been received. Our team will confirm within <strong>30 minutes</strong>.</p>
      <div style="background:var(--green-bg);border:1px solid #bbf7d0;border-radius:var(--radius);padding:.875rem;margin:.75rem 0;text-align:left">
        <div style="font-size:13px;font-weight:600;color:var(--green);margin-bottom:4px">✅ Notifications sent automatically:</div>
        <div style="font-size:12px;color:var(--green)">📧 Order receipt emailed to <strong>${esc(email)}</strong></div>
        <div style="font-size:12px;color:var(--green);margin-top:3px">📱 WhatsApp message sent to <strong>${esc(phone)}</strong></div>
        <div style="font-size:12px;color:var(--green);margin-top:3px">🔔 Store owner has been notified</div>
      </div>
...
function sendWA(order) {
  ...
  // ── Message to STORE OWNER (you receive this on WhatsApp) ──
  const ownerMsg = encodeURIComponent(
    ...
    `📧 *Email:* ${order.email}\n`+
    `📍 *Address:* ${order.addr}\n`+
    `🚚 *Delivery:* ${order.delivery==='delivery'?'Home Delivery (GHS '+order.deliveryFee+')':'Store Pickup (Free)'}\n\n`+
    `📦 *Items Ordered:*\n${items}\n\n`+
    `💰 *Subtotal:* GHS ${fmt(order.subtotal)}\n`+
    `🚚 *Delivery:* GHS ${order.deliveryFee}\n`+
    `✅ *TOTAL: GHS ${fmt(order.total)}*\n\n`+
    `📝 *Notes:* ${order.notes||'None'}\n\n`+
    `⏰ Please confirm this order within 30 minutes.`
  );

  // Open WhatsApp to YOUR number with the order details
  window.open(`https://wa.me/${STORE.phone}?text=${ownerMsg}`, '_blank');
}

function sendCustomerReceipt(order) {
  // ── Receipt sent to CUSTOMER on WhatsApp ──
  const items = order.items.map(i=>`  • ${i.name} ×${i.qty} — GHS ${fmt(i.price)}`).join('\n');

  // Clean customer phone — remove spaces, dashes, and leading +
  let custPhone = order.phone.replace(/[\s\-\(\)]/g,'');
  if (custPhone.startsWith('+')) custPhone = custPhone.slice(1);
  if (custPhone.startsWith('0')) custPhone = '233' + custPhone.slice(1);

  const custMsg = encodeURIComponent(
    `✅ *ORDER CONFIRMED — ${order.orderId}*\n`+
    `Chris & Louisa Tech Store\n`+
    `Shiashi, Accra, Ghana\n\n`+
    `Dear ${order.name},\n`+
    `Thank you for your order! Here is your receipt:\n\n`+
    `📦 *Items:*\n${items}\n\n`+
    `💰 Subtotal: GHS ${fmt(order.subtotal)}\n`+
    `🚚 Delivery: ${order.deliveryFee>0?'GHS '+order.deliveryFee:'Free (Pickup)'}\n`+
    `✅ *TOTAL: GHS ${fmt(order.total)}*\n\n`+
    `📍 *Delivery:* ${order.delivery==='delivery'?'Home delivery to: '+order.addr:'Store pickup — Shiashi, Accra'}\n\n`+
    `We will contact you on ${order.phone} within 30 minutes to confirm.\n\n`+
    `Questions? WhatsApp us: +233 547 472 867\n`+
    `Email: nasiruchristiantalamah@gmail.com`
  );

  // Send receipt to customer's WhatsApp
  setTimeout(() => {
    window.open(`https://wa.me/${custPhone}?text=${custMsg}`, '_blank');
  }, 1500); // slight delay so owner message opens first
}
...
const ADMIN_PASSWORD = "chris2025";
...
function openAdmin()  { promptAdmin(); }
