...
async function sendOwnerEmail(order) {
  const itemsList = order.items.map(i =>
    `${i.name} x${i.qty} — GHS ${fmt(i.price)} each = GHS ${fmt(i.price * i.qty)}`
  ).join('\n');
  ...
  try {
    await emailjs.send(EMAILJS.serviceId, EMAILJS.ownerTpl, params);
...
async function sendCustomerEmail(order) {
  ...
  // Use same template but send to customer
  const params = {
    order_id:         order.orderId,
    customer_name:    order.name,
    customer_phone:   order.phone,
